...
def define_graph():
    ...
    lstm_hidden_unit_list = [256, 512, 64]
    ...
    # change MAX_IN_REVIEW
    # 

    # trying using relu as the fc layer.
    ...
    LSTM_cell = [tf.nn.rnn_cell.LSTMCell(num, forget_bias=1.0, name="LSTM"+ str(idx), initializer=tf.orthogonal_initializer()) for idx, num in enumerate(lstm_hidden_unit_list)]
    LSTM_drop = [tf.nn.rnn_cell.DropoutWrapper(
                    cell = c,
                    output_keep_prob = dropout_keep_prob
                    ) for c in LSTM_cell[:-1]]
