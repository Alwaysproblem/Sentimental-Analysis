...
def define_graph():
    ...
    # 256 0.005 the accuracy is 0.83
    ...
    lstm_fw = tf.nn.rnn_cell.LSTMCell(hidden_unit_fw, forget_bias= 1.0, initializer = tf.orthogonal_initializer())
    lstm_bw = tf.nn.rnn_cell.LSTMCell(hidden_unit_bw, forget_bias= 1.0, initializer = tf.orthogonal_initializer())
    # lstm_fw_drop = tf.nn.rnn_cell.DropoutWrapper(lstm_fw, output_keep_prob=dropout_keep_prob)
    # lstm_bw_drop = tf.nn.rnn_cell.DropoutWrapper(lstm_bw, output_keep_prob=dropout_keep_prob)

    # lstm_fw = tf.nn.rnn_cell.GRUCell(
    #                 hidden_unit_fw,
    #                 kernel_initializer=tf.random_normal_initializer(),
    #                 bias_initializer=tf.random_normal_initializer()
    #                 )
    # lstm_bw = tf.nn.rnn_cell.GRUCell(
    #                 hidden_unit_bw,
    #                 kernel_initializer=tf.random_normal_initializer(),
    #                 bias_initializer=tf.random_normal_initializer()
    #                 )
    # lstm_fw_drop = tf.nn.rnn_cell.DropoutWrapper(lstm_fw, output_keep_prob=dropout_keep_prob)
    # lstm_bw_drop = tf.nn.rnn_cell.DropoutWrapper(lstm_bw, output_keep_prob=dropout_keep_prob)
    lstm_fw_drop = lstm_fw
    lstm_bw_drop = lstm_bw



    (value_fw,value_bw), _ = tf.nn.bidirectional_dynamic_rnn(
                                cell_fw =lstm_fw_drop, 
                                cell_bw =  lstm_bw_drop, 
                                dtype = tf.float32, 
    ...
    pred_128_norm = tf.layers.batch_normalization(pred_128)
    ...
    pred_64_norm = tf.layers.batch_normalization(pred_64)
